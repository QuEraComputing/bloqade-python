...
from quera_ahs_utils.quera_ir.task_results import QuEraTaskResults, QuEraTaskStatusCode, QuEraShotResult, QuEraShotStatusCode
...
def simulate_task_results(task: QuEraTaskSpecification,  p_full = 0.99, p_empty = 0.01):
    ...
    pre_sequence_probs = np.array([p_full if fill==1 else p_empty for fill in filling])
    ...
    for shot in range(task.nshots):
        ...
        post_sequence = rng.binomial(np.ones(natoms,dtype=int), pre_sequence*post_sequence_probs)
