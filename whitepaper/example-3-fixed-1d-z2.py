...
fixed_1d_z2_prog = (
    start
    .add_positions([(0.0, i * cast("lattice_constant")) for i in range(n_atoms)])
    .rydberg.rabi.amplitude.uniform.piecewise_linear(
        durations, rabi_amplitude_values
    )
    .detuning.uniform.piecewise_linear(
        durations, rabi_detuning_values
    )
)

fixed_1d_z2_job = (
    fixed_1d_z2_prog
    .assign(lattice_constant=6.1, sweep_time=2.4)
    .braket_local_simulator(10000)
)

fixed_1d_z2_report = fixed_1d_z2_job.submit().report()
...
rydberg_densities = fixed_1d_z2_report.rydberg_densities()
...
for i in range(n_atoms):
    for j in range(n_atoms):
        correlation_table[i, j] = ((1 - report.dataframe.iloc[:, i]) * (1 - report.dataframe.iloc[:, j])).mean() - rydberg_densities.iloc[0, i] * rydberg_densities.iloc[0, j]
